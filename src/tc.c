...
void system_setup(){
    st.pointer = 0;
    regs.status = 0x2;
    regs.pc = 0x0400; // ROM ENTRY POINT
}
...
int main(int argc, char **argv){
    ...
    if (argc > 3)
    {
        printf("Usage: %s <rom-image> [mon|run]", argv[0]);
        return 1;
    }
    ...
    printf("\nComputation Terminated: %d cycles elapsed", cycles);
}
