...
void Video_computeFontTranslationTable(DeviceVideo *v, Font font)
{
    ...
    for (int i = 0; i < 256; i++) {
        ...
        v->renderer.fontTranslationTable[i] =
...
void VideoNextFont(DeviceVideo *v, const char *path)
{
    ...
    v->charbuffer_w =
        (u8)((float)v->renderer.screen_texture->texture.width / em.x);
    v->charbuffer_h =
        (u8)((float)v->renderer.screen_texture->texture.height / em.y);
    ...
    v->renderer.characters_texture =
        LoadRenderTexture(v->charbuffer_w, v->charbuffer_h);
    SetTextureFilter(v->renderer.characters_texture.texture,
                     TEXTURE_FILTER_POINT);
    v->renderer.charsFake =
        MemAlloc((size_t)v->charbuffer_w * v->charbuffer_h * sizeof(Color));
...
void VideoChangeFont(DeviceVideo *v, u8 fontIndex)
{
    if (v->fonts[fontIndex].glyphCount != 0) {
        ...
        v->charbuffer_w =
            (u8)(v->renderer.screen_texture->texture.width /
                 MeasureTextEx(*v->font, "M", (float)v->font->baseSize, 0.0f).x);
        v->charbuffer_h = (u8)(v->renderer.screen_texture->texture.height /
                               v->font->baseSize);
        Video_computeFontTranslationTable(v, *v->font);
...
void Video_RendererInit(DeviceVideo *v, const char *path)
{
    ...
    v->renderer.cursorChar_loc =
        GetShaderLocation(v->renderer.shader, "cusorChar");
    v->renderer.baseColor_loc =
        GetShaderLocation(v->renderer.shader, "baseColor");
    ...
    v->renderer.palettebg_loc =
        GetShaderLocation(v->renderer.shader, "paletteBg");
    v->renderer.time_loc = GetShaderLocation(v->renderer.shader, "uTime");
    v->renderer.charSize_loc =
        GetShaderLocation(v->renderer.shader, "charSize");
    v->renderer.textureSize_loc =
        GetShaderLocation(v->renderer.shader, "mainTextureSize");

    SetShaderValue(v->renderer.shader, v->renderer.cursorChar_loc,
                   &v->cursorChar, SHADER_UNIFORM_INT);
    SetShaderValue(v->renderer.shader, v->renderer.baseColor_loc,
                   &v->renderer.foreground_color, SHADER_UNIFORM_IVEC4);
    SetShaderValueV(v->renderer.shader, v->renderer.palette_loc,
                    v->renderer.shaderPalette, SHADER_UNIFORM_IVEC4, 16);
    SetShaderValueV(v->renderer.shader, v->renderer.palettebg_loc,
                    v->renderer.shaderPaletteBg, SHADER_UNIFORM_IVEC4, 8);
}

static void ClearScreen(TaleaMachine *m)
{
    memset(&m->main_memory[m->video.framebuffer_addr], 0,
           (size_t)m->video.framebuffer_h * (size_t)m->video.framebuffer_w *
               m->video.bpp);
    memset(&m->main_memory[m->video.charbuffer_addr], ' ',
           (size_t)m->video.charbuffer_h * (size_t)m->video.charbuffer_w *
               m->video.bpc);
};
...
void Video_Update(TaleaMachine *m)
{
    if (m->video.mode == VIDEO_MODE_TEXT_MONO ||
        m->video.mode == VIDEO_MODE_TEXT_COLOR ||
        m->video.mode == VIDEO_MODE_TEXT_AND_GRAPHIC) {
        for (size_t i = 0; i < m->video.charbuffer_h; i++) {
            for (size_t j = 0, k = 0;
                 j < (size_t)m->video.charbuffer_w * m->video.bpc;
                 j += m->video.bpc, k++) {
                u8 character =
                    m->video.renderer.fontTranslationTable
                        [m->main_memory[TALEA_CHARBUFFER_ADDR +
                                        ((size_t)m->video.charbuffer_w *
                                         m->video.bpc * i) +
                                        j]];
                ...
                if (m->video.mode == VIDEO_MODE_TEXT_COLOR ||
                    ...
                    u8 rawColor =
                        m->main_memory[TALEA_CHARBUFFER_ADDR +
                                       ...
                                       j + 1];
                    m->video.renderer
                        .charsFake[(m->video.charbuffer_w * i) + k] = (Color){
                        character,
                        0,        // reserved
                        0,        // reserved
                    ...
                    m->video.renderer
                        .charsFake[(m->video.charbuffer_w * i) + k] =
    ...
    if (m->video.mode == VIDEO_MODE_GRAPHIC ||
        m->video.mode == VIDEO_MODE_TEXT_AND_GRAPHIC) {
        UpdateTexture(m->video.renderer.framebuffer.texture,
                      &m->main_memory[TALEA_FRAMEBUFFER_ADDR]);
        DrawTexturePro(
            m->video.renderer.framebuffer.texture,
            (Rectangle){ 0.0, 0.0f,
                         (float)m->video.renderer.framebuffer.texture.width,
    ...
    if (m->video.mode == VIDEO_MODE_TEXT_MONO ||
        ...
        UpdateTexture(m->video.renderer.characters_texture.texture,
                      m->video.renderer.charsFake);

        float charSize[2]    = { MeasureTextEx(*m->video.font, "M",
                                               (float)m->video.font->baseSize,
                                               0.0f)
                                     .x,
                                 (float)m->video.font->baseSize };
        int   textureSize[2] = {
            m->video.renderer.screen_texture->texture.width,
            m->video.renderer.screen_texture->texture.height
        };
        int cursorIndex =
            m->video.renderer.fontTranslationTable[m->video.cursorChar];
        BeginShaderMode(m->video.renderer.shader); // Enable our custom shader
                                                   // for next shapes/textures
                                                   // drawings
        SetShaderValue(m->video.renderer.shader,
                       m->video.renderer.cursorChar_loc, &cursorIndex,
                       SHADER_UNIFORM_INT);
        SetShaderValue(m->video.renderer.shader,
                       m->video.renderer.baseColor_loc,
                       &m->video.renderer.foreground_color,
                       SHADER_UNIFORM_IVEC4);
        SetShaderValue(m->video.renderer.shader, m->video.renderer.charSize_loc,
                       &charSize, SHADER_UNIFORM_VEC2);
        SetShaderValue(m->video.renderer.shader,
                       m->video.renderer.textureSize_loc, &textureSize,
                       SHADER_UNIFORM_IVEC2);
        SetShaderValueTexture(m->video.renderer.shader,
                              m->video.renderer.font_loc,
                              m->video.font->texture);
        SetShaderValueTexture(m->video.renderer.shader,
                              m->video.renderer.chars_loc,
...
{
    printf("[VIDEO] Written 0x%x to 0x%04x\n", value, addr);
    if (addr == P_VIDEO_COMMAND) {
        ...
        case COMMAND_SETMODE:
            ...
            if (m->video.mode == VIDEO_MODE_TEXT_MONO) {
                ...
                m->video.bpc = 1;
                Video_RendererInit(&m->video,
                                   SHADERS_PATH("mode_text_mono.fs"));
            } else {
                UnloadShader(m->video.renderer.shader);
                m->video.bpc = 2;
                Video_RendererInit(&m->video,
                                   SHADERS_PATH("mode_text_color.fs"));
            };
            break;
        case COMMAND_SETFONT:
            m->video.font = &m->video.fonts[P_VIDEO_DATAH];
            break;
        case COMMAND_BLIT:
            // not implemented
            break;
        case COMMAND_SETFB:
            m->video.framebuffer_addr =
                (u32)m->data_memory[P_VIDEO_DATAH] << 16 |
                (u32)m->data_memory[P_VIDEO_DATAM] << 8 |
                m->data_memory[P_VIDEO_DATAL];
            break;
        case COMMAND_SETFB_SZ: {
            u16 w = (u16)m->data_memory[P_VIDEO_DATAH] << 8 |
                    m->data_memory[P_VIDEO_DATAM];
            u16 h = (u16)m->data_memory[P_VIDEO_GPU0] << 8 |
                    m->data_memory[P_VIDEO_GPU1];
            m->video.framebuffer_w = (w) ? w : m->video.framebuffer_w;
            m->video.framebuffer_h = (h) ? h : m->video.framebuffer_h;
            break;
        }
        case COMMAND_SET_BPP:
            m->video.bpp = m->data_memory[P_VIDEO_DATAH];
            break;
        case COMMAND_SETCB:
            m->video.charbuffer_addr = (u32)m->data_memory[P_VIDEO_DATAH]
                                           << 16 |
                                       (u32)m->data_memory[P_VIDEO_DATAM] << 8 |
                                       m->data_memory[P_VIDEO_DATAL];
            break;
        ...
        case COMMAND_SET_BPC:
            m->video.bpc = m->data_memory[P_VIDEO_DATAH];
            break;
        case COMMAND_SETVBLANK:
            m->video.vblank_enable = m->data_memory[P_VIDEO_DATAH];
            break;
        case COMMAND_LOADFONT: {
            ...
            int fontSize = (u32)m->data_memory[P_VIDEO_GPU0] << 24 |
                           ...
                           (u32)m->data_memory[P_VIDEO_GPU2] << 8 |
                           m->data_memory[P_VIDEO_GPU3];

            SaveFileData(VIDEO_FONT_FILE_HACK, fontData, fontSize);
            m->video.fonts[m->video.next_font++] =
                LoadFont(VIDEO_FONT_FILE_HACK);
        ...
        case COMMAND_SETBGCOLOR:
            m->video.renderer.background_color = (Color){
                m->data_memory[P_VIDEO_GPU0], m->data_memory[P_VIDEO_GPU1],
                m->data_memory[P_VIDEO_GPU2], m->data_memory[P_VIDEO_GPU3]
            };
            break;
        case COMMAND_SETFGCOLOR:
            m->video.renderer.foreground_color = (Color){
                m->data_memory[P_VIDEO_GPU0], m->data_memory[P_VIDEO_GPU1],
                m->data_memory[P_VIDEO_GPU2], m->data_memory[P_VIDEO_GPU3]
            };
            break;
        case COMMAND_CLEARREGS:
            memset(&m->data_memory[DEV_VIDEO_BASE], 0, 16);
            break;
        case COMMAND_FONTINFO:
            ...
            m->data_memory[P_VIDEO_STATUS1] =
                (int)MeasureTextEx(*m->video.font, "M",
                                   (float)m->video.font->baseSize, 0.0f)
...
void Video_Reset(TaleaMachine *m, TaleaConfig *config, bool is_restart)
{
    ...
    VideoNextFont(&m->video,
                  TextFormat("%s%s", FONT_PATH, config->hardware_font));
