...
size_t(fwrite)(const void *ptr, size_t size, size_t nelem, FILE *str)
{ /* write to stream from array */
    char  *s  = (char *)ptr;
    size_t ns = size * nelem;

    if (ns == 0) return (0);
    while (0 < ns) { /* ensure room in buffer */
        if (str->_Next < str->_Wend)
            ;
        else if (_Fwprep(str) < 0)
            break;
        { /* copy in as many as possible */
            char  *s1 = str->_Mode & _MLBF ? (char *)memchr(s, '\n', ns) : NULL;
    ...
    if (str->_Mode & _MNBF) { /* disable and drain */
        str->_Wend = str->_Buf;
        fflush(str);
    }
