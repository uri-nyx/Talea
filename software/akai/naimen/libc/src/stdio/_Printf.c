...
{ /* print formatted */
    ...
    for (x.nchar = 0;;) { /* scan format string */
        const char *s = fmt;

        { /* copy any literal text */
            int n = 0;
            // TODO: maybe use the wide characters thingy
            while (*s && *s != '%') s++; 

            n = s - fmt;
            PUT(fmt, n);
            if (*s == '\0') return (x.nchar);
            fmt = s++;
        }
        { /* parse a conversion specifier */
            ...
            for (x.flags = 0; (t = strchr(fchar, *s)) != NULL; ++s) x.flags |= fbit[t - fchar];
            if (*s == '*') { /* get width argument */
                ...
                for (x.width = 0; isdigit(*s); ++s)
                    if (x.width < _WMAX) x.width = x.width * 10 + *s - '0';
            if (*s != '.')
                x.prec = -1;
                ...
                for (x.prec = 0; isdigit(*s); ++s)
                    if (x.prec < _WMAX) x.prec = x.prec * 10 + *s - '0';
