...
{ /* get a line from stream */
    ...
    if (n <= 1) return (NULL);
    for (s = (unsigned char *)buf; 0 < --n && str->_Nback;) { /* deliver pushed back chars */
        *s = str->_Back[--str->_Nback];
        if (str->_Nback == 0) str->_Rend = str->_Rsave;
        if (*s++ == '\n') { /* terminate full line */
            *s = '\0';
            return (buf);
        }
    }
    while (0 < n) { /* ensure buffer has chars */
