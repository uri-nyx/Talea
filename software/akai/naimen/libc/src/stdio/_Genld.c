...
{ /* generate long double text */
    const char point = '.'; //TODO: locale localeconv()->decimal_point[0];

    if (nsig <= 0) nsig = 1, p = "0";
    if (code == 'f' || (code == 'g' || code == 'G') && -4 <= xexp && xexp < px->prec) { /* 'f'
                                                                                           format */
        ...
        if (xexp <= 0) { /* digits only to right of point */
            px->s[px->n1++] = '0';
            if (0 < px->prec || px->flags & _FNO) px->s[px->n1++] = point;
            ...
            px->n1 += nsig;
            px->nz1 = xexp - nsig;
            if (0 < px->prec || px->flags & _FNO) px->s[px->n1] = point, ++px->n2;
            px->nz2 = px->prec;
        ...
        if (code == 'g' || code == 'G') { /* fixup for 'g' */
            if (nsig < px->prec && !(px->flags & _FNO)) px->prec = nsig;
