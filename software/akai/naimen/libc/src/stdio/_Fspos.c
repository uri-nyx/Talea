...
/* macros */
#define NNL(str, m1, m2) ((str)->_Mode & _MBIN ? 0 : 0)

int ak_seek(int fd, long offset, int whence); /* Akai system call */

int _Nnl(const unsigned char *, const unsigned char *);

long _Fspos(FILE *str, const fpos_t *ptr, long loff, int way)
{                                                             /* position a file */
    if (!(str->_Mode & (_MOPENR | _MOPENW)) || fflush(str)) { /* not-open or write error */
        errno = EFPOS;
        return (EOF);
    }
    if (ptr) loff += ptr->_Off; /* fsetpos */
    ...
    if (loff == -1) { /* request failed */
        ...
        if (str->_Mode & (_MREAD | _MWRITE)) { /* empty buffer */
            str->_Next  = str->_Buf;
            str->_Rend  = str->_Buf;
            str->_Wend  = str->_Buf;
            str->_Nback = 0;
