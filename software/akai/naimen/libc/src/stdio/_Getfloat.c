...
{ /* get a floating point value for _Scanf */
    ...
    p = ac, ch = GETN(px);
    _trace(0xDFAAD123, ch);
    if (ch == '+' || ch == '-') *p++ = ch, ch = GETN(px);
    for (; 0 <= ch && isdigit(ch); seendig = 1) *p++ = ch, ch = GETN(px);
    if (ch == '.' /* TODO: locale localeconv()->decimal_point[0]*/) *p++ = ch, ch = GETN(px);
    for (; 0 <= ch && isdigit(ch); seendig = 1) *p++ = ch, ch = GETN(px);
    if ((ch == 'e' || ch == 'E') && seendig) { /* parse exponent */
        *p++ = ch, ch = GETN(px);
        if (ch == '+' || ch == '-') *p++ = ch, ch = GETN(px);
        for (seendig = 0; 0 <= ch && isdigit(ch); seendig = 1) *p++ = ch, ch = GETN(px);
    }
    UNGETN(px, ch);
    if (!seendig) return (p == ac && ch < 0 ? EOF : 0);
    *p = '\0';
    if (!px->noconv) { /* convert and store */
        double dval = strtod(ac, NULL);
        _trace(0xDFAAD124, ac[0], dval);
