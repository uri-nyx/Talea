...
{ /* convert long double to text */
    ...
    if (px->prec < 0)
        ...
        px->prec = 1;
    if (0 < (errx = _Ldunscale(&xexp, &px->v.ld))) { /* x == NaN, x == INF */
        ...
        { /* convert significant digits */
            int gen = px->prec + (code == 'f' ? xexp + 2 + NDIG : 2 + NDIG / 2);

            if (LDBL_DIG + NDIG / 2 < gen) gen = LDBL_DIG + NDIG / 2;
            for (*p++ = '0'; 0 < gen && 0.0 < ldval; p += NDIG) { /* convert NDIG at a time */
                int  j;
                long lo = (long)ldval;

                if (0 < (gen -= NDIG)) ldval = (ldval - (long double)lo) * 1e8L;
            ...
            gen = p - &ac[1];
            for (p = &ac[1], xexp += NDIG - 1; *p == '0'; ++p) --gen, --xexp; /* correct xexp */
