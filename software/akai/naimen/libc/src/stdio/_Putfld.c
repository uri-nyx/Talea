...
/* macros */
#define LDSIGN(x) (((*(const unsigned long *)&(x)) >> 31) & 1)
...
{ /* convert a field for _Printf */
    ...
    case 'c': /* convert a single character */ ac[px->n0++] = va_arg(*pap, int); break;
    case 'd':
    case 'i': /* convert a signed decimal integer */
        px->v.li = px->qual == 'l' ? va_arg(*pap, long) : va_arg(*pap, int);
        if (px->qual == 'h') px->v.li = (short)px->v.li;
    ...
    case 'X': /* convert unsigned */
        ...
        if (px->flags & _FNO && px->v.li != 0) { /* indicate base with prefix */
            ac[px->n0++] = '0';
    ...
    case 'e':
    case 'E':
    case 'f': /* convert floating */
    case 'g':
    case 'G':
#ifndef _AKAI_STDIO_NO_FLOAT_
    ...
    case 's': /* convert a string */
        ...
        px->n1 = strlen(px->s);
        if (0 <= px->prec && px->prec < px->n1) px->n1 = px->prec;
        break;
    case '%': /* put a '%' */ ac[px->n0++] = '%'; break;
    default: /* undefined specifier, print it out */ ac[px->n0++] = code;
    }
}
