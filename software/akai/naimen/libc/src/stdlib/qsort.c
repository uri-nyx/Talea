...
{                   /* sort (char base[size])[n] using quicksort */
    while (1 < n) { /* worth sorting */
        ...
        while (i < j) { /* partition about pivot */
            while (i < j && (*cmp)(qi, qp) <= 0) ++i, qi += size;
            while (i < j && (*cmp)(qp, qj) <= 0) --j, qj -= size;
            if (i < j) { /* swap elements i and j */
                ...
                for (ms = size; 0 < ms; ms -= m, q1 += m, q2 += m) { /* swap as many as possible
                                                                      */
        ...
        if (j < i) { /* recurse on smaller partition */
            if (1 < j) qsort(qi, j, size, cmp);
            n = i;
        } else { /* lower partition is smaller */
            if (1 < i) qsort(base, i, size, cmp);
            base = qi;
            n    = j;
        }
    }
