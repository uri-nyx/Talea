...
#define MAXLINE (sizeof(int) == 2 ? 256 : 4096)
#define MAXPAT                                 \
    256 /* max number of chars per replacement \
         * pattern */
/* max file name size */
...
void prntln(str, vflg, lin) char *str;
int  vflg, lin;
{
    ...
    while (*str && *str != NL) {
        if (*str < ' ' || *str >= 0x7f) {
            ...
            default: putcntl(*str, stdout); break;
            }
        } else
            putc(*str, stdout);
        str++;
...
int         nflg, lflg, promptflg;
int         line1, line2, nlines;
extern char fname[];
int         version = 50;
int         diag    = 1;

void intr(sig) int sig;
{
    printf("?\n");
    longjmp(env, 1);
}
...
char **argv;
{
    ...
    usize first_file = 0;
    char *cmd        = NULL;
    int   doexit = FALSE, oneshot = FALSE, dosave = FALSE, dosave_append = FALSE;
    int   res = 0;

    set_buf();
    doflush = isatty(PDEV_STDOUT);

    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-") == 0 || strcmp(argv[i], "-s") == 0) {
            diag = 0;
            continue;
        }
        if (strcmp(argv[i], "-Q") == 0) {
            /* quit after one command is run without saving (only when -c is passed)*/
            oneshot = TRUE;
            continue;
        }
        if (strcmp(argv[i], "-q") == 0) {
            /* save and quit after one command is run (only when -c is passed)*/
            oneshot = TRUE;
            dosave  = TRUE;
            continue;
        }
        if (strcmp(argv[i], "-qW") == 0) {
            /* append and quit after one command is run (only when -c is passed)*/
            oneshot       = TRUE;
            dosave        = TRUE;
            dosave_append = TRUE;
            continue;
        ...
        if (strcmp(argv[i], "-c") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "%s: -c requires a string command\n", argv[0]);
                return EXIT_FAILURE;
            }
            cmd = argv[++i];
            continue;
        }
        /* first non-option: start of filenames */
        first_file = i;
        break;
    }
    ...
    while (1) {
        ...
        if (doexit) {
            if (dosave) {
                if (first_file < argc) {
                    for (i = first_file; i < argc; i++) {
                        if (deflt(1, lastln) < 0) return (EXIT_FAILURE);
                        if (dowrite(line1, line2, argv[i], dosave_append) < 0)
                            return (EXIT_FAILURE);
                    }
                }
            }
        ...
        if (doflush) {
            if (promptflg && diag) putchar(PROMPT);
            fflush(stdout);
        }

        if (cmd && strncpy(inlin, cmd, sizeof(inlin))) {
            strncat(inlin, "\n", sizeof(inlin));
            doexit = oneshot;
            cmd    = NULL;
        } else if (fgets(inlin, sizeof(inlin), stdin) == NULL) {
            break;
        }
        ...
        for (;;) {
            ...
            if (inptr >= inlin + 2 && inptr[-2] == '\\' && inptr[-1] == NL) {
                inptr[-1] = 'n';
                if (fgets(inptr, sizeof(inlin) - (inptr - inlin), stdin) == NULL) break;
            } else {
                break;
            }
        }
...
int getlst()
{
    ...
    // fprintf(stderr, "getlst: nlines=%d line1=%d line2=%d\n", nlines, line1, line2);
...
int first;
{
    ...
    switch (c = *inptr) {
    case '.': inptr++; return (curln);

    case '$': inptr++; return (lastln);
    ...
    default: return (first ? EOF : 1); /* unknown address */
    }
}
...
int    ret_endp;
{
    ...
    while (*line) {
        ...
        if ((rval = amatch(line2, pat2, bptr)) == 0) {
            ...
            if (rval > bptr && rval > line) rval--; /* point to last char matched */
...
int num;
{
    ...
    if (line1 <= 0 || line2 < line1 || (line1 <= num && num <= line2)) return (ERR);
...
struct tbl {
    ...
    "number",   &nflg,        TRUE,      "nonumber", &nflg,    FALSE,      "list",
    &lflg,      TRUE,         "nolist",  &lflg,      FALSE,    "eightbit", &eightbit,
    TRUE,       "noeightbit", &eightbit, FALSE,      "prompt", &promptflg, TRUE,
    "noprompt", &promptflg,   FALSE,     0
};
...
int set()
{
    ...
    for (i = 0; *inptr != SP && *inptr != HT && *inptr != NL;) word[i++] = *inptr++;
...
int show()
{
    ...
    printf("number %s, list %s, prompt %s\n", nflg ? "ON" : "OFF", lflg ? "ON" : "OFF",
           promptflg ? "ON" : "OFF");
    return (0);
}
...
void relink(a, x, y, b) LINE *a, *x, *y, *b;
{
    x->l_prev = a;
    y->l_next = b;
}
...
char *c;
{
    ...
    pid = ak_rfork(RF_MEM_SHARE | RF_FIL_CLEAN | RF_LEASE_STDERR | RF_LEASE_STDIN |
                       RF_LEASE_STDOUT | RF_PARENT_KEEP_RUNNING,
                   0);
    ...
    if (pid < 0)
        ...
        args[1] = c;
        args[2] = NULL;
        ak_exec(SHELL, 2, args, O_EXEC_GUESS);
        exit(-1);
    }
...
void unmakepat(head) TOKEN *head;
{
