/*
 * tuibox.h: simple tui library. Ported to the AKAI OS
 */
...
#define ui_loop(u) \
    char buf[64];  \
    int  n;        \
    while ((n = ak_dev_ctl(PDEV_STDIN, PX_READ, buf, sizeof(buf))) > 0)
...
void ui_new(int s, ui_t *u)
{
    ...
    u32 info     = ak_dev_ctl(DEV_TEXTBUFFER, TCTL_GET_INFO, NULL, 0);
    u->ws.ws_col = (info >> 8) & 0xff;
    u->ws.ws_row = info & 0xff;

    u->tio = ak_dev_ctl(PDEV_STDIN, PX_GETCANON, NULL, 0);
    raw[0] = u->tio;
    raw[0] &= ~(IN_ECHO | IN_CANON);
    raw[0] |= IN_BLOCK | IN_MIN1;
    ak_dev_ctl(PDEV_STDIN, PX_SETCANON, &raw, 1);
    ...
    printf("\x1b[0m\x1b[2J\x1b[?1006h\x1b[?25l");
...
void ui_free(ui_t *u)
{
    ...
    printf("\x1b[0m\x1b[2J\x1b[?1006l\x1b[?25h");
    ak_dev_ctl(PDEV_STDIN, PX_SETCANON, &(u->tio), 1);
 ...
 *   strip this down.
 */
int ui_add(int x, int y, int w, int h, int screen, char *watch, char initial, func draw,
           func onclick, func onhover, void *data1, void *data2, ui_t *u)
{
