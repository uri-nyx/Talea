...
static void bios_init(void)
{
    ...
    Memsize   = ((u32)_lbud(REG_SYSTEM_MEMSIZE_FLASH) * (1 << 16)) + (1 << 16);
...
{
    extern void palette(int argc, char **argv);
    /*
                SYSTEM INFORMATION
    ...
    */
    ...
    day   = _lbud(REG_SYSTEM_DAY);
    month = _lbud(REG_SYSTEM_MONTH);
    year  = _lbud(REG_SYSTEM_YEAR);
    ...
    ttty_mux_printf(&Con, "Color mode: %s [MODE %x]\n\n",
                    ...
                    Video_mode);
    if (Video_mode != VIDEO_MODE_TEXT_MONO) palette(0, NULL);
    ttty_putc(&Video_tty, '\n');
    ttty_mux_puts(&Con, "[ CORE ]\n");

    /* Processor */
    /* TODO: Do a bit of fixed point */
    khz = 10000 + (GET_SYSTEM(REG_SYSTEM_CLOCK) * 353);
    ttty_mux_printf(&Con, "Processor:\t\t%s (%s) @ %dKhz\n",
                    (GET_SYSTEM(REG_SYSTEM_ARCH_ID) == ARCH_ID) ? "Sirius" : "unknown",
                    (GET_SYSTEM(REG_SYSTEM_VENDOR_ID) == VENDOR_ID) ? "of the House of Talea" :
                                                                      "of unknown origin",
                    khz);
...
{
    ttty_mux_puts(&Con, "\x1b[2J\x1b[H");
}
...
void ser(int argc, char *argv[])
{
    ...
    while (true) {
        ...
        if (serial_available()) {
            ...
            _trace(0x88, count);
            for (i = 0; i < count; i++) {
                u8 c = _lbud(Devices.tty + SER_RX);
                _trace(0x89, c);
                if (c != '\b') ttty_putc(&Video_tty, c);
            }
        }
    };
}
...
void mouse(int argc, char *argv[])
{
    ...
    for (sprite_data_addr = 0x1200; sprite_data_addr < (0x1200 + 64); sprite_data_addr++) {
        _sbd(sprite_data_addr, mouse_pointer_sprite[sprite_data_addr - 0x1200]);
    }

    _shd(Devices.mouse + MOUSE_SPRITE, 0x1200);
...
{
    _sbd(REG_SYSTEM_MINUTE, TALEA_SYSTEM_MILLIS_MODE); // set counter mode
...
#define TPS_SECTOR_DATA_ADDR (0x1400)
...
static void io_stress_test(int ioctl)
{
    ...
    if (fatal) return;
    {
        time_accum = time_accum ? time_accum : 1;
        ttty_mux_printf(&Con,
...
{
    u32 time;
    _sbd(REG_SYSTEM_YEAR, TALEA_SYSTEM_UNIXTIME_MODE); // unixtime mode
    time = _lwd(REG_SYSTEM_YEAR);
    _sbd(REG_SYSTEM_YEAR, TALEA_SYSTEM_CALENDAR_MODE); // date mode
    return time;
}
...
void clock(int argc, char *argv[])
{
    ...
    do {
        u32 unix = unixtime();
        u8  ss   = _lbud(REG_SYSTEM_SECOND);
        u8  mm   = _lbud(REG_SYSTEM_MINUTE);
        u8  hh   = _lbud(REG_SYSTEM_HOUR);
...
void fontload(int argc, char *argv[])
{
    ...
    _copymd(testfont_data, 0x1800, sizeof(testfont_data));
    _sbd(Devices.video + VIDEO_COMMAND, COMMAND_BEGIN_DRAWING);
    _sbd(Devices.video + VIDEO_GPU0, 0);               // load into base font slot
    _shd(Devices.video + VIDEO_GPU1, 0x1800);          // address in DATA of the pallete
    _sbd(Devices.video + VIDEO_GPU3, 255);             // element count -1
    _sbd(Devices.video + VIDEO_GPU4, 0);               // starting elment index
    _sbd(Devices.video + VIDEO_GPU5, '?');             // default elment index
    _sbd(Devices.video + VIDEO_GPU6, 8);               // char w
    _sbd(Devices.video + VIDEO_GPU7, 16);              // char w
    _sbd(Devices.video + VIDEO_COMMAND, COMMAND_LOAD); // fire!
    _sbd(Devices.video + VIDEO_COMMAND, COMMAND_END_DRAWING);
    ...
    Video_tty.w              = cols;
    Video_tty.h              = rows;
    Video_tty.default_att[0] = 0x0;
    Video_tty.default_att[1] = 0xf;
    Video_tty.default_att[2] = 0x0;
    ttty_clear(&Video_tty);
}

void shutdown(int argc, char *argv[])
{
    ttty_mux_printf(&Con, "To shutdown press enter. Any other key to exit\n");
...
static const ushell_command_t commands[] = {
    ...
    { "shutdown", "shuts down the machine", &shutdown },
